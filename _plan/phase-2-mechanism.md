...
- **Direct-suite follow-up (2026-08-28; `shinychat#dy7g`):** bypassing the
  ...
  files changed.
- **Clarified private settlement-callback invariant (2026-08-28; roborev job
  1019):** each consumer task lineage holds an immutable `ContextVar` tuple
  stack of `(source Chat, exact consumers delivery)`. Destructive operations
  reject while any stacked delivery remains identity-present in its source
  Chat's pending FIFO, regardless of the target `Chat`. A copied child context
  naturally becomes allowed once all inherited entries dequeue. FIFO topology
  remains one deque, one runner, and one wake-up.
...
- **Nested settlement context handoff (2026-08-28; `a08c6def`; roborev job
  1019 closed):** nested response settlement deliveries now retain their
...
- **Completion and next handoff (2026-08-28; `shinychat#47fa`):** the browser
  `${id}_messages:shinychat.messages` echo was removed atomically with the
  Python handler, input ID, parser, and obsolete bookmark exclusion. The
  transcript is server-owned; forged tagged legacy input cannot mutate the
  transcript, history, or bookmark state. Normal user input retains its nonce
  and event priority; `shinychat#dy7g` settlement is unchanged. Implementation
  commits are `5a156651`, `83757b96`, `c180d7d2`, and review-fix `f89c005`.
  JS lint/tests passed (1238 tests). Full `py-check` had 191 Playwright and
  649 non-browser passes plus one skip; its only failure was the pre-existing
  `shinychat#4z6p` Markdown MIME assertion. Focused follow-up checks passed;
  `js/dist` equals the Python packaged assets, and R assets are unchanged
