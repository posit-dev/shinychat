...
# warns when `display` is not a list
    ...
    Condition
      ...
      i You can also use `markdown` or `text` items in `display` to show Markdown or plain text, respectively.
    Output
      <shiny-tool-result request-id="test-id" tool-name="test-tool" request-call="`test-tool`()" arguments="[]" status="success" show-request value="{}" value-type="code" data-shinychat-react></shiny-tool-result>
