...
* `chat_server()` gets multi-conversation history automatically: a drawer for starting new chats and returning to previous ones, with LLM-generated titles, search, rename, and delete. Conversations are persisted per-user (or a custom scope) via a pluggable store — the default `FileConversationStore` finds a redeploy-safe location automatically on Posit Connect. Customize with `history = history_options(...)` — e.g. how the active conversation is restored across reloads (browser storage, URL, or Shiny bookmarking) and callbacks to keep app state synced to it — or opt out with `history = FALSE`. The history object also offers programmatic control, including a reactive conversation ID (also emitted on OpenTelemetry spans as `gen_ai.conversation.id`) and an explicit `save()` method. For apps that can't use `chat_server()`, wire it up manually with `chat_enable_history()`. (#266, #307, #328)

* You can now edit a message after sending it. Hover a user message (or press and hold on a touch device) and click the pencil icon to edit and resend. Editing forks the conversation from that point — the original branch is kept as a sibling, and `‹ 1 / 2 ›` controls let you switch between versions at any time, including after reloading the page or returning from the history drawer. Requires history to be enabled (the default with `chat_server()`). (#269)
...
* Added file attachment support: users can upload images, PDFs, and text files alongside chat messages via a file picker button, drag-and-drop, or clipboard paste. `chat_server()` enables attachments by default and automatically converts uploads into ellmer `Content` objects for the model. For non-`chat_server()` usage, enable with `allow_attachments = TRUE` (or a MIME allow-list). When attachments are enabled, `input$<id>_user_input` is a list of ellmer `Content` objects (typed text, if present, followed by one object per attachment) rather than a plain string — forward it to a chat method by splicing with `!!!`, e.g. `chat$stream_async(!!!input$<id>_user_input)`; the `last_input` reactive returned by `chat_server()` mirrors this shape. The maximum combined attachment size defaults to approximately 30 MB and can be configured via the `SHINYCHAT_MAX_ATTACHMENT_SIZE` environment variable. (#250)
...
* A tool's definition `title` (from its annotations) and its result `title` (from `tool_result_display()`) are now shown as-is, without client-side tense conjugation — the old `"Running {title}"` / `"{title} failed"` templates are gone. The definition title shows while the call is running; for a single-call row, the result title (if provided) replaces it when the result arrives. If a title now reads oddly while running, use an explicit present-tense definition title (e.g. "Running R code") and, optionally, a past-tense result title (e.g. "Ran R code"). Failures are shown via a separate status cue rather than appended to the title.
