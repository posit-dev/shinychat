...
* Fixed a cross-site scripting vulnerability in message rendering. shinychat's
  internal custom elements (`<shiny-chat-raw-html>`, `<shiny-tool-request>`,
...
* `chat_ui(messages = )` now renders htmltools/Shiny UI correctly again. Such
  content was labelled as markdown, so its internal `<shiny-chat-raw-html>`
...
* `output_markdown_stream()` now labels non-string `content` (e.g. UI with
  ...
  internal `<shiny-chat-raw-html>` wrapper displayed as literal text and the
  UI never bound.
