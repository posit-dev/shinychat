---
title: "Customizing Tool UI in shinychat"
output:
  html_document:
    theme:
      version: 5
...
vignette: >
  %\VignetteIndexEntry{Customizing Tool UI in shinychat}
...
This strategy is so useful that shinychat automatically supports it for a `.tool_intent` argument.
When `.tool_intent` is present in the tool's arguments, shinychat shows this value in the tool request and result titles as the _reason the tool was called_.

To demonstrate, we'll use take a break from our weather tool and create a tool that simply generates a random number.
On it's own, the tool doesn't need any arguments, but we include a `.tool_intent` argument in the tool function and we include a description to explain its purpose.

```{r}
#| echo: true

tool_random_number <- tool(
  function(.tool_intent) {
    runif(1)
  },
  name = "tool_random_number",
  description = "Generate a random number.",
  arguments = list(
    .tool_intent = type_string(
...
request_random_number <- ContentToolRequest(
  id = "tool_call_006",
  name = "tool_random_number",
  arguments = list(.tool_intent = "Generate a random number for testing"),
