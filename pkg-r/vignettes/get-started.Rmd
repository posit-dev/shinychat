...
`chat_server()` or `chat_enable_history()`. Use `chat_artifact_update()`,
`chat_artifact_hide()`, and `chat_artifact_toggle()` for subsequent artifact
...
`toolbar = NULL`, which omits the scoped segment. Supply UI to replace that
segment. For compatibility with earlier code, `toolbar = TRUE` reuses the home
toolbar and `toolbar = FALSE` is an alias for omitting it. Use
`toolbar_global` for actions that remain mounted across every page; it is
rendered after the scoped segment and preserves its Shiny input and output
state while controls move between desktop and mobile layouts. On narrow
screens, navigation and toolbar controls move into the app menu above the
active page's sidebar content without duplicating IDs.

The repository includes [paired runnable navigation and artifact-control
examples](https://github.com/posit-dev/shinychat/tree/main/examples/page-chat).
