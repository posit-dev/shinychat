...
2. Create a fresh `ellmer::stream_controller()` for each request and store it in a `reactiveVal()`.
3. Pass the controller to `chat$stream_async()` via the `controller` argument.
4. Observe `input$<id>_cancel` (where `<id>` is your chat element's ID) and call `ctrl$cancel()` when it fires.
...
server <- function(input, output, session) {
  ...
  current_controller <- reactiveVal(NULL)
