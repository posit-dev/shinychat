...
server <- function(input, output, session) {
  ...
  chat$slash_command("search", "Search the docs", function(args) {
    chat$append(paste("Searching for:", args))
  })
}
...
- Handlers take 0 or 1 argument (the text after the command name). They're just app code, so they can show modals, trigger downloads, update reactive values, append messages, or do nothing visible at all.
...
<div class="callout">
  <div class="title">Bookmarking</div>
  <div class="body">
  Unlike the Python package, the R chat module restores its conversation from the underlying ellmer client's turns rather than from a separate record of what was displayed. Because a slash command's visible message can differ from what (if anything) was sent to the model, slash command messages are **not** restored when a bookmarked app is reopened.
  </div>
</div>
