...
The `chat_mod_ui()` and `chat_mod_server()` functions enable file attachments by default. Uploaded files are automatically converted to ellmer content objects and sent to the model — no extra code needed.
...
- The maximum combined attachment size defaults to 30 MB and can be configured via the `SHINYCHAT_MAX_ATTACHMENT_SIZE` environment variable.