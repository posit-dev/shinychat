...
segment, or use `toolbar = TRUE` for the legacy behavior that moves the home
toolbar into the selected page. `toolbar = FALSE` remains a legacy alias for
no scoped toolbar. Use `toolbar_global` for actions that remain mounted across
every page; it is rendered after the scoped segment and preserves its Shiny
input and output state while controls move between desktop and mobile layouts.
