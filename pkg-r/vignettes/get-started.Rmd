...
Use `chat$transform_user_input()` when the model needs extra context for each
message. This works well for retrieval-augmented generation and apps where the
model acts on the current document or dashboard state.

The callback receives the submitted contents and must return the contents that
the chat client sends to the model. Add context before the submitted contents
so the model receives both. The chat UI and `chat$last_input()` still show the
message that the user submitted.
...
`contents` has the same shape as `input$chat_user_input`: a string when
attachments are disabled, or a list of ellmer content objects when attachments
are enabled. You can register more than one callback. shinychat runs callbacks
in the order that you register them. Slash commands don't go through
`transform_user_input()`: the command's handler owns the transformation for
its submissions, as described in [Slash commands](#slash-commands).
