...
Slash commands give users discoverable shortcuts -- like `/clear`, `/help`, or `/save` -- that run a handler you define on the server. They're a good fit for "meta" actions that aren't really a message to the model: resetting the conversation, opening a help dialog, exporting a transcript, and so on.

Register commands on the object returned by `chat_mod_server()`, using its `slash_command()` method. When a user runs a command, its handler fires instead of the text being sent to the model.

### Zero-argument handlers

A handler that takes no arguments is called when the command is invoked. This is useful for actions that don't need any user input:
...
ui <- page_fillable(
  chat_mod_ui("chat", messages = list("Type `/` to see available commands."))
)
...
### Handlers that accept input

A handler that takes one argument receives a `ContentSlashCommand` object -- not a plain string. This object carries the command name, the text typed after it, and a `text` property that controls what the LLM sees. For example, when a user types `/search shiny modules`:

- `content@command` is `"search"`
- `content@args` is `"shiny modules"`
- `content@text` is a descriptive default (e.g., `"The user entered the /search slash command with arguments: shiny modules"`)

Set `content@text` to whatever the LLM should actually see, then pass the object to `client$stream()` or `client$chat()`:

```r
chat$slash_command("search", "Search the docs", function(content) {
  content@text <- paste(
    "Search the documentation for the following topic:", content@args
  )
...
Because `ContentSlashCommand` extends `ellmer::ContentText`, it works anywhere a `ContentText` does. LLM providers read the `text` property, while the chat UI shows the original `/search shiny modules` on restore. This means that when the chat is restored from a bookmark (or pre-existing turns), the user sees the original slash command they typed rather than the transformed text that was sent to the LLM.
...
- A command's handler runs instead of sending the text to the model. A `/` message that doesn't match any registered command is sent as an ordinary message.
...
- The `echo` argument controls whether invoking a command is shown as a user message and awaits a response. It defaults to `TRUE` when a handler is given. Pass `echo = FALSE` for a handler that only performs side effects (e.g. opening a modal) and shouldn't appear in the conversation.

Slash commands are currently available only through the chat module (`chat_mod_server()`), not when building a custom UI with `chat_ui()` directly.
...
event and call `preventDefault()` to skip the server round-trip:

```r
chat$slash_command("clear", "Clear the input", NULL)  # no server handler

tags$script(HTML("
  document.addEventListener('shiny:chat-slash-command', function(e) {
    if (e.detail.id !== 'chat-chat' || e.detail.command !== 'clear') return;
    e.preventDefault();
...
Inside a module, the chat container id is namespaced: `chat_mod_ui("chat")`
produces a container with id `chat-chat`, which is the value passed as
`e.detail.id`. Use that namespaced id (or read `e.detail.id` directly) when
targeting a specific chat instance.
...
The easiest way to add cancellation support is to use the `chat_mod_ui()` and `chat_mod_server()` functions. The module handles everything automatically — the stop button is shown during streaming and wired up internally, with no extra code required.
...
If you are building a custom chat UI with `chat_ui()` directly, you can enable cancellation by setting `enable_cancel = TRUE` and wiring up the cancel input in your server function.
