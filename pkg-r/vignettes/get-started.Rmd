...
`chat_server()` or `chat_enable_history()`. Use `chat_artifact_panel_update()`,
`chat_artifact_panel_hide()`, and `chat_artifact_panel_toggle()` for subsequent artifact
...
segment. Supply UI to add a page-specific toolbar. Use `toolbar_global` for
actions that remain mounted across every page; it is
