...
The event is cancelable and bubbles. Use `e.detail.id` to target a specific chat. `preventDefault()` skips the server round-trip; set `e.detail.echo` to control whether the command appears as a user message. Inside a module, the chat container id is namespaced (`chat_mod_ui("chat")` produces `chat-chat`).
