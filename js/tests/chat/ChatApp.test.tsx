...
describe("user submission", () => {
  it("sends exactly one user input without a messages snapshot", async () => {
    ...
    const calls = mockSetInputValue.mock.calls
    expect(calls[0]?.[0]).toBe("chat_user_input:shinychat.userInput")
    expect(calls.some(([id]) => id.includes("_messages"))).toBe(false)
  })
})
