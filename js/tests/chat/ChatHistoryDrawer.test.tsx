...
const DEFAULT_CONVOS: ConversationMeta[] = [
  ...
  makeConvo({ id: "c", title: "Old chat", updated_at: daysAgo(5) }),
]

function renderDrawer(props: Partial<ChatHistoryDrawerProps> = {}) {
...
describe("conversation list grouping", () => {
  ...
  it("shows only Previous group when all conversations are older", () => {
    renderDrawer({
      conversations: [
        makeConvo({ id: "x", title: "A", updated_at: daysAgo(2) }),
        makeConvo({ id: "y", title: "B", updated_at: daysAgo(3) }),
      ],
    })
    openDrawer()
