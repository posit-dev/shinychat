...
* A new `<shiny-sidenote>` markup convention: any assistant message can include an inline `<shiny-sidenote label="..." url="...">markdown shown in the popover</shiny-sidenote>` tag anywhere in its markdown, and it renders as a small pill at the end of the paragraph or list item it's attached to. Hovering, clicking, or focusing the pill opens a popover showing the sidenote's body; when a block contains more than one sidenote, the popover becomes a carousel with previous/next controls. Sidenotes sharing a `label` collapse into one pill with a paged popover; the `+N` overflow badge appears only when the grouped sidenotes have differing labels — see the `Sidenotes` callout in the `append_message`/`append_message_stream` docs for the full convention. This is a general-purpose building block any LLM can be prompted to emit directly; it isn't tied to any particular content source.
