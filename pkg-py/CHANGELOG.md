...
* `Chat.messages()` is now server-authoritative and updates in the same reactive flush as `Chat.append_message()`. The server retains the settled transcript for history and persistence; the browser is a rendering and interaction projection. (#272)
