...
* A new `<shiny-aside>` markup convention: any assistant message can include an inline `<shiny-aside>` tag anywhere in its markdown. The tag renders as a small pill at the end of its paragraph or list item. Hovering, clicking, or focusing the pill opens a popover that shows the aside body. Labeled asides in one block collapse into a paged popover. Each unlabeled aside remains a separate numbered pill. The `+N` overflow badge appears only when grouped labeled asides have different labels. A safe `url` makes the source heading a link and supplies its derived favicon. The optional `grounded-span` attribute links an aside to the most recent exact preceding match in the same block. Shinychat highlights that text while the aside is open. Set `SHINYCHAT_ASIDE_FAVICON=false` to prevent the third-party favicon request. Long content wraps and scrolls within the viewport. Scoped Bootstrap themes remain applied, and popover page changes are announced to assistive technology. See the `Asides` callout in the `append_message` and `append_message_stream` documentation for the full convention. This general-purpose building block is not tied to a particular content source.
