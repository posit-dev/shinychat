...
* A new `<shiny-aside>` markup convention: any assistant message can include an inline `<shiny-aside label="..." url="...">markdown shown in the popover</shiny-aside>` tag anywhere in its markdown, and it renders as a small pill at the end of the paragraph or list item it's attached to. Hovering, clicking, or focusing the pill opens a popover showing the aside's body; when a block contains more than one aside, the popover becomes a carousel with previous/next controls. Asides sharing a `label` collapse into one pill with a paged popover; the `+N` overflow badge appears only when the grouped asides have differing labels — see the `Asides` callout in the `append_message`/`append_message_stream` docs for the full convention. This is a general-purpose building block any LLM can be prompted to emit directly; it isn't tied to any particular content source.
