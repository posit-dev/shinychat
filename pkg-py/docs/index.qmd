...
Use `@chat.on_user_input` with `Chat(client=...)` when the model needs extra
context for each message. This works well for retrieval-augmented generation
and apps where the model acts on the current document or dashboard state.

The callback receives the arguments that shinychat passes to
...
You can register more than one callback. shinychat runs them in registration
order.
