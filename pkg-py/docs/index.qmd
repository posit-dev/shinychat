...
Use `@chat.transform_user_input` with `Chat(client=...)` when the model needs
extra context for each message. This works well for retrieval-augmented
generation and apps where the model acts on the current document or dashboard
state.

The function receives the arguments that shinychat passes to
`client.stream_async()`. Return the arguments that the client should receive.
Add context before the submitted contents so the model receives both. The chat
...
The list starts with the submitted text. Any attachment content follows it.
You can register more than one function. shinychat runs them in registration
order. Slash commands don't go through `transform_user_input()`: a command's
