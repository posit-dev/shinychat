...
|-----------|----------------------------------------------------------------------------------|------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|------------|
| id        | [str](`str`)                                                                     | A unique identifier for the chat session. In Shiny Core, make sure this id matches a corresponding :func:`~shiny.ui.chat_ui` call in the UI.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         | _required_ |
| messages  | [Sequence](`typing.Sequence`)\[[Any](`typing.Any`)\]                             | A sequence of messages to display in the chat. A given message can be one of the following:  * A string, which is interpreted as markdown and rendered to HTML on the client.     * To prevent interpreting as markdown, mark the string as       :class:`~shiny.ui.HTML`. * A UI element (specifically, a :class:`~shiny.ui.TagChild`).     * This includes :class:`~shiny.ui.TagList`, which take UI elements       (including strings) as children. In this case, strings are still       interpreted as markdown as long as they're not inside HTML. * A dictionary with `content` and `role` keys. The `content` key can contain a   content as described above, and the `role` key can be "assistant" or "user".  **NOTE:** content may include specially formatted **input suggestion** links (see `.append_message()` for more information). | `()`       |
| on_error  | [Literal](`typing.Literal`)\[\'auto\', \'actual\', \'sanitize\', \'unhandled\'\] | How to handle errors that occur in response to user input. When `"unhandled"`, the app will stop running when an error occurs. Otherwise, a notification is displayed to the user and the app continues to run.  * `"auto"`: Sanitize the error message if the app is set to sanitize errors,   otherwise display the actual error message. * `"actual"`: Display the actual error message to the user. * `"sanitize"`: Sanitize the error message before displaying it to the user. * `"unhandled"`: Do not display any error message to the user.                                                                                                                                                                                                                                                                                                  | `'auto'`   |
| tokenizer | [TokenEncoding](`shinychat._chat_tokenizer.TokenEncoding`) \| None               | The tokenizer to use for calculating token counts, which is required to impose `token_limits` in `.messages()`. If not provided, a default generic tokenizer is attempted to be loaded from the tokenizers library. A specific tokenizer may also be provided by following the `TokenEncoding` (tiktoken or tozenizers) protocol (e.g., `tiktoken.encoding_for_model("gpt-4o")`).                                                                                                                                                                                                                                                                                                                                                                                                                                                                    | `None`     |

## Attributes

| Name | Description |
| --- | --- |
| [latest_message_stream](#shinychat.Chat.latest_message_stream) | React to changes in the latest message stream. |

## Methods

| Name | Description |
| --- | --- |
| [append_message](#shinychat.Chat.append_message) | Append a message to the chat. |
| [append_message_stream](#shinychat.Chat.append_message_stream) | Append a message as a stream of message chunks. |
| [clear_messages](#shinychat.Chat.clear_messages) | Clear all chat messages. |
| [destroy](#shinychat.Chat.destroy) | Destroy the chat instance. |
| [enable_bookmarking](#shinychat.Chat.enable_bookmarking) | Enable bookmarking for the chat instance. |
| [message_stream_context](#shinychat.Chat.message_stream_context) |         Message stream context manager. |
| [messages](#shinychat.Chat.messages) | Reactively read chat messages |
| [on_user_submit](#shinychat.Chat.on_user_submit) | Define a function to invoke when user input is submitted. |
| [set_user_message](#shinychat.Chat.set_user_message) | Deprecated. Use `update_user_input(value=value)` instead. |
| [transform_assistant_response](#shinychat.Chat.transform_assistant_response) | Transform assistant responses. |
| [transform_user_input](#shinychat.Chat.transform_user_input) | Transform user input. |
| [update_user_input](#shinychat.Chat.update_user_input) | Update the user input. |
| [user_input](#shinychat.Chat.user_input) | Reactively read the user's message. |

### append_message { #shinychat.Chat.append_message }
...
### append_message_stream { #shinychat.Chat.append_message_stream }
...
### clear_messages { #shinychat.Chat.clear_messages }
...
### destroy { #shinychat.Chat.destroy }
...
### enable_bookmarking { #shinychat.Chat.enable_bookmarking }
...
### message_stream_context { #shinychat.Chat.message_stream_context }
...
### messages { #shinychat.Chat.messages }
...
### on_user_submit { #shinychat.Chat.on_user_submit }
...
### set_user_message { #shinychat.Chat.set_user_message }
...
### transform_assistant_response { #shinychat.Chat.transform_assistant_response }
...
### transform_user_input { #shinychat.Chat.transform_user_input }
...
### update_user_input { #shinychat.Chat.update_user_input }
...
### user_input { #shinychat.Chat.user_input }
