...
def test_list_populates_cache(store: FileConversationStore):
    async def _run() -> None:
...
def test_list_returns_from_cache_without_disk_read(
    store: FileConversationStore, tmp_path: Path
):
    async def _run() -> None:
        rec = new_conversation_record(title="cached")
...
def test_put_updates_warm_cache(store: FileConversationStore):
    async def _run() -> None:
...
def test_put_does_not_create_cache_for_cold_scope(store: FileConversationStore):
    async def _run() -> None:
...
def test_delete_updates_warm_cache(store: FileConversationStore):
    async def _run() -> None:
        rec = new_conversation_record(title="t")
        await store.put("alice", rec)
        await store.list("alice")  # warm
...
def test_list_returns_independent_copy(store: FileConversationStore):
    async def _run() -> None:
        rec = new_conversation_record(title="t")
        await store.put("alice", rec)
...
def test_memory_put_get_round_trip(mem_store: InMemoryConversationStore):
    async def _run() -> None:
        rec = new_conversation_record(title="penguins")
        rec.append_linear({"role": "user", "content": "hi"})
...
def test_memory_get_missing_returns_none(mem_store: InMemoryConversationStore):
    async def _run() -> None:
...
def test_memory_list_newest_first(mem_store: InMemoryConversationStore):
    async def _run() -> None:
        a = new_conversation_record(title="older")
        b = new_conversation_record(title="newer")
...
def test_memory_scopes_are_isolated(mem_store: InMemoryConversationStore):
    async def _run() -> None:
        rec = new_conversation_record(title="private")
        await mem_store.put("alice", rec)
...
def test_memory_delete(mem_store: InMemoryConversationStore):
    async def _run() -> None:
        rec = new_conversation_record(title="t")
        await mem_store.put("alice", rec)
...
def test_memory_search(mem_store: InMemoryConversationStore):
    async def _run() -> None:
        a = new_conversation_record(title="Q1 churn analysis")
        b = new_conversation_record(title="Penguin bills")
        await mem_store.put("alice", a)
...
def test_resolve_store_auto_dev_mode_returns_in_memory(monkeypatch: pytest.MonkeyPatch):
    monkeypatch.setenv("SHINY_DEV_MODE", "1")
    assert isinstance(resolve_store("auto"), InMemoryConversationStore)


def test_resolve_store_auto_no_dev_mode_returns_file(monkeypatch: pytest.MonkeyPatch):
    monkeypatch.delenv("SHINY_DEV_MODE", raising=False)
    assert isinstance(resolve_store("auto"), FileConversationStore)


def test_resolve_store_passthrough_custom_store(mem_store: InMemoryConversationStore):
...
def test_file_store_total_size_zero_for_missing_scope(tmp_path: Path):
    async def _run() -> None:
...
def test_file_store_total_size_grows_with_put(tmp_path: Path):
    async def _run() -> None:
        store = FileConversationStore(dir=tmp_path)
        rec = new_conversation_record(title="t")
...
def test_file_store_total_size_shrinks_with_delete(tmp_path: Path):
    async def _run() -> None:
        store = FileConversationStore(dir=tmp_path)
...
def test_memory_total_size_zero_for_missing_scope(mem_store: InMemoryConversationStore):
    async def _run() -> None:
...
def test_memory_total_size_grows_with_put(mem_store: InMemoryConversationStore):
    async def _run() -> None:
        rec = new_conversation_record(title="t")
        rec.append_linear({"role": "user", "content": "hello world"})
        await mem_store.put("alice", rec)
...
def test_memory_total_size_shrinks_with_delete(mem_store: InMemoryConversationStore):
    async def _run() -> None:
        rec1 = new_conversation_record(title="a")
        rec2 = new_conversation_record(title="b")
