...
def test_url_mode_switch_restores_inputs(
    page: Page, local_app: ShinyAppProc
) -> None:
    """Switching conversations navigates to a bookmark URL and restores
    Shiny input values — the capability browser mode cannot provide."""
...
def test_url_mode_reload_returns_to_conversation(
    page: Page, local_app: ShinyAppProc
) -> None:
    """The URL tracks the active conversation, so a plain reload restores
    transcript AND inputs."""
    page.goto(local_app.url)
...
def test_url_mode_stale_state_id_falls_back_to_draft(
    page: Page, local_app: ShinyAppProc
) -> None:
