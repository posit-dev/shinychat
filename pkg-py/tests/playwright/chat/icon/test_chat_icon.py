...
class ChatModule:
    def __init__(self, page: Page, id: str, classes: str):
        self.id = id
        self.chat = ChatController(page, f"chat_{id}")
        self.classes = classes

    def expect_last_message_icon_to_have_classes(
        self, classes: Optional[str] = None
    ):
        last_msg_icon = self.chat.loc_latest_message.locator(
            ".message-icon > *"
        ).first
        expect(last_msg_icon).to_have_class(
            classes or self.classes, timeout=30 * 1000
        )
