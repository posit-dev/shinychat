...
def test_navigation_example_saves_conversations(
    ...
    expect(page_chat.loc).to_be_visible()
...
def test_navigation_example_mobile_menu_includes_home_link(
    ...
    page.goto(local_app.url)
    page_chat = PageChatController(page, "chat")
    home_link = page_chat.loc.locator(".shiny-chat-page-home-link")
    chat = ChatController(page, "chat")
    ...
    sidebar_box = page_chat.loc_sidebar.bounding_box()
    sources_box = page_chat.loc.get_by_role(
        "button", name="Sources"
    ).bounding_box()
...
def test_navigation_example_shows_settings_offcanvas_from_global_toolbar(
    ...
    page.goto(local_app.url)
    page_chat = PageChatController(page, "chat")
    settings_button = page_chat.loc.locator("#show_settings")
    help_button = page_chat.loc.locator("#help")
    ...
    expect(
        offcanvas.get_by_role("button", name="Reset settings")
    ).to_be_visible()
