...
def test_generate_title_uses_custom_callable():
    async def titler(turns):
        return "  My   Title  "

    result = asyncio.run(
        generate_title(titler, None, [{"role": "user", "content": "q"}])
    )
    assert result == "My Title"


def test_generate_title_supports_sync_callable():
    def titler(turns):
        return "Sync Title"

    assert asyncio.run(generate_title(titler, None, [])) == "Sync Title"


def test_generate_title_failure_returns_none():
    async def titler(turns):
    ...
    with pytest.warns(
        ...
        result = asyncio.run(generate_title(titler, None, []))
    assert result is None


def test_generate_title_none_returning_fn_returns_none():
    def titler(turns):
        return None

    assert asyncio.run(generate_title(titler, None, [])) is None


def test_generate_title_none_fn_non_chatlas_client_returns_none():
    class NotChatlas: ...

    assert asyncio.run(generate_title(None, NotChatlas(), [])) is None
